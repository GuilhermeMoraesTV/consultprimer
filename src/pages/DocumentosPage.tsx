...
import {
  ...
  CheckCircle2, Clock, XCircle, Filter, Package, X,
} from 'lucide-react';
...
function SemaforoValidade({ doc }: { doc: Documento }) {
  if (!doc.dataValidade) {
    return <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><CheckCircle2 className="w-3.5 h-3.5" /> Sem vencimento</span>;
  }
  const dias = diasRestantes(doc.dataValidade);
  if (dias < 0) return <span className="inline-flex items-center gap-1 text-xs text-status-pendente font-semibold"><XCircle className="w-3.5 h-3.5" /> Vencido há {Math.abs(dias)} dias</span>;
  if (dias <= 30) return <span className="inline-flex items-center gap-1 text-xs text-status-analise font-semibold"><AlertTriangle className="w-3.5 h-3.5" /> Vence em {dias} dias</span>;
  return <span className="inline-flex items-center gap-1 text-xs text-status-ganha font-medium"><CheckCircle2 className="w-3.5 h-3.5" /> Válido ({dias} dias)</span>;
}
      ...
      <div className="relative w-full max-w-3xl bg-card border-l border-border shadow-2xl animate-slide-in flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">{TIPO_DOCUMENTO_LABELS[doc.tipo]} • {(doc.tamanhoKb / 1024).toFixed(1)} MB</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <Button variant="outline" size="sm" className="gap-1.5"><Download className="w-3.5 h-3.5" /> Baixar</Button>
        ...
        <div className="flex-1 flex items-center justify-center bg-muted/30 p-8">
          <div className="bg-card rounded-xl shadow-lg p-12 max-w-md text-center border border-border/50">
            ...
            <p className="text-sm text-muted-foreground mb-4">Conecte ao Lovable Cloud com Supabase Storage para visualizar.</p>
...
export default function DocumentosPage() {
  ...
  const certidoesAlerta = documentos.filter((d) => d.dataValidade && (d.status === 'vencido' || d.status === 'proximo_vencer'));
    ...
    <AppLayout titulo="Gestão de Documentos (GED)" subtitulo="Repositório centralizado com controle de validade">
            ...
            {certidoesAlerta.map((d) => (
              <button key={d.id} onClick={() => setDocVisualizar(d)}
                className={cn(
                  ...
                  d.status === 'vencido' ? 'bg-status-pendente/10 border-status-pendente/30 text-status-pendente' : 'bg-status-analise/10 border-status-analise/30 text-status-analise'
                )}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 animate-fade-in">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="gap-1.5" onClick={() => toast({ title: '📦 Download em Lote', description: 'Conecte ao Cloud para gerar o ZIP.' })}>
      ...
      <div className="border-2 border-dashed border-border/60 rounded-xl p-6 mb-5 text-center hover:border-primary/40 transition-colors cursor-pointer glass-card animate-fade-in">
        ...
        <p className="text-xs text-muted-foreground">PDF, DOC, XLS • Máx 20MB</p>
      </div>
          ...
          {filtrados.map((doc) => (
            <div key={doc.id}
              ...
              <div className={cn('w-9 h-9 rounded-lg flex items-center justify-center shrink-0',
                doc.status === 'vencido' ? 'bg-status-pendente/10' : doc.status === 'proximo_vencer' ? 'bg-status-analise/10' : 'bg-primary/10'
              )}>
                <FileText className={cn('w-4 h-4',
                  doc.status === 'vencido' ? 'text-status-pendente' : doc.status === 'proximo_vencer' ? 'text-status-analise' : 'text-primary'
                )} />
              </div>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button className="p-1.5 rounded hover:bg-secondary"><Download className="w-4 h-4 text-muted-foreground" /></button>
              </div>
            </div>
          ))}
        </div>
